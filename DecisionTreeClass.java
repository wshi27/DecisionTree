...
public class DecisionTreeClass {
	...
	public HeapPriorityQueue<Integer, DecisionTreeNode> heap = 
			new HeapPriorityQueue<Integer, DecisionTreeNode>();
			...
			for(int feature = 0; feature < train_data.num_features; feature++) {
				...
				if(currNode.opt_fea_type == 0) {
					...
					for(int z = 0; z < sortedFeature.length-1; z++) {
						...
						if((higherNumAbove + higherNumBelow) > score) {
							score = higherNumAbove + higherNumBelow;
 ...
 */
	private boolean testHelper(ArrayList<String> resultList, DecisionTreeNode node) {
		System.out.println(resultList.remove(0));
		System.out.println(node.opt_fea_id);
		if(node.opt_fea_id != Integer.parseInt(resultList.remove(0))) 
			return false;

		System.out.println(node.opt_improvement);
		if(node.opt_improvement != Integer.parseInt(resultList.remove(0))) 
			return false;

		System.out.println(node.is_leaf);
		if(node.is_leaf != Boolean.parseBoolean(resultList.remove(0))) 
			return false;

		System.out.println(node.majority_class);
		if(node.majority_class != Integer.parseInt(resultList.remove(0)))
			return false;

		System.out.println(node.num_accurate);
		if(node.num_accurate != Integer.parseInt(resultList.remove(0))) 
			return false;
		if(node.opt_fea_type == 1) {
			System.out.println(node.split_class_id);
			if(node.split_class_id != Integer.parseInt(resultList.remove(0)))
				return false;
		}
		else {
			System.out.println(node.opt_fea_thd);
